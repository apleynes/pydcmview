...
class ImageLoader:
    ...
    def _validate_file(self):
        ...
        if self.file_path.suffix.lower() not in self.SUPPORTED_EXTENSIONS:
            # Check for .nii.gz
            if not (self.file_path.suffix.lower() == '.gz' and 
                   self.file_path.with_suffix('').suffix.lower() == '.nii'):
    ...
    def apply_window_level(self, array: np.ndarray, center: float, width: float) -> np.ndarray:
